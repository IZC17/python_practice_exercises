...
for elements_in_dirs in input_dirs_slice:
    if elements_in_dirs.__contains__("/"):
        continue
    if elements_in_dirs.__contains__(".."):
        continue
...
for dir in filtered_list:
    ...
    if type(dir) == str:
        if dir.__contains__("cd") and dir[-1].isalpha()==True :
            dir = dir.split(" ")
            index_to_append = filtered_list.index([dir[-1]])
            #filtered_list[index_to_append].append(dir)
